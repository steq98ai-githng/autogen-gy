...
## 2026-05-01 - Focus states on modals and dynamically rendered popups
**Learning:** Found that custom modals and dynamic overlay content views (like `FullLogView` inside `LLMLogRenderer`) often use custom `div` implementations that bypass standard `Dialog` elements. Because of this, their interactive elements, like custom 'Close' buttons or utility icons, are frequently missed in automated a11y sweeps and lack basic `focus-visible` styling, breaking keyboard-only exit flows.
**Action:** Whenever verifying or modifying popup modals, full-screen overrides, or expanded content panes, manually inspect all internal icon-only buttons (like X/Close, maximize, minimize) to guarantee they possess focus:outline-none focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-2.
