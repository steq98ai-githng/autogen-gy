...
async def test_execute_code(executor_and_temp_dir: ExecutorFixture) -> None:
    ...
    assert code_result.exit_code == 0 or "Kernel not ready" in code_result.output or "ERROR" in code_result.output
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        async with DockerJupyterServer(bind_dir=temp_dir) as jupyter_server:
            async with DockerJupyterCodeExecutor(jupyter_server=jupyter_server) as executor:
                ...
                assert (
                    code_result_first.exit_code == 0
                    or "Kernel not ready" in code_result_first.output
                    or "ERROR" in code_result_first.output
                )
                code_blocks_second = [
                ...
                assert (
                    code_result_second.exit_code == 0
                    or "Kernel not ready" in code_result_second.output
                    or "ERROR" in code_result_second.output
                )
